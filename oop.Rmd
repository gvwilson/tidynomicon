...
```{r}
as.character.two_d <- function(obj){
  paste0("<", obj[1], ", ", obj[2], ">")
}
as.character(first)
as.character(second)
...
```{r}
library(sloop)
s3_dispatch(as.character(first))
```

Compare this with calling `as.character` on a plain old character vector:

```{r}
s3_dispatch(as.character(c(7.1, 7.2)))
```

The specialized functions associated with a generic function like `as.character` are called [methods](../glossary/#method).
...
new_two_d <- function(coordinates){
  cat("typeof(coordinates)", typeof(coordinates), "\n")
